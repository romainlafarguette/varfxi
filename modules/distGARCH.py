...
class DistGARCH(object):
    ...
    def __init__(self, depvar_str, data, level_str=None, 
        ...
        # Fix the random seed to have consistent random sampling
        np.random.seed(seed=19830525) # My gf birthday :-)
...
class DistGARCHForecast(object): # Forecast class for the DistGARCHFit class
    ...
    def VaR_FXI(self, qv_l=[0.05, 0.95]):
        """ 
    ...
    def plot_conditional_cdf(self, q_low=0.05, q_high=0.95,
        ...
        if thresholds_t:
            ...
            dv = self.fixed_thresholds_FXI(thresholds_t)
            dclose = dclose.merge(dv[['FXI']], left_index=True, right_index=True)
    ...
    def pit_plot(self,
        ...
        ci_u = [x+1.34*len(pits)**(-0.5) for x in support]
        ...
        ax.plot(support, ci_u, color='red', label='5 percent critical values',
                linestyle='dashed')
